...
function collector(
  opts: Omit<Parameters<typeof createTimelineCollector>[0], "toolTiming" | "getConfig"> & {
  ...
  return createTimelineCollector({ ...rest, getConfig, toolTiming })
}
...
describe("createTimelineCollector (event-driven)", () => {
  ...
  test("reset clears memory but not write behavior", () => {
    const c = collector({
      config: { ...DEFAULT_TIMELINE, enabled: true },
      getSessionId: () => "r",
      append: async () => {},
